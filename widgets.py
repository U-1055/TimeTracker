...
from base import (COLOR1, COLOR3, COMMON_FONT, DAY_ROWS, CBOX_DEFAULT, START_TEXT, STOP_TEXT, COMMON_FONT_COLOR, CURRENT_DEED,
                  TIME_MAIN, TIME_DEED, NAME, TIME_START, TIME_END, TIME, time_to_sec) #ToDo: перевести на константы
...
class StopWatchSelector(Frame):
    ...
    def place_widgets(self):
        ...
        self.wdg_selector = ComboBox(self, self.values)  # ToDo: Разобраться с лямбдами
        ...
        self.start_btn = CTkButton(self, text=START_TEXT, state=NORMAL, command=self.start)
    ...
    def start(self):
        ...
        self.thread_1 = Thread(target=self.count_time, args=[self.wdg_main_swatch])
        self.thread_1.start()
        self.thread_2 = Thread(target=self.count_time, args=[self.wdg_deed_swatch])
        self.thread_2.start()

        self.counting = True
        self.running = True  # Потоки запущены
...
class DeedsPanel(Frame):
    ...
    def add_deed(self, deeds: tuple[str, str, str], deed_color: str):
        ...
        name, time_start, time_end = deeds[NAME], deeds[TIME_START], deeds[TIME_END]
